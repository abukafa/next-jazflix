...
export default function AddMovieForm({ genres = [] }) {
  ...
  const [tmdbInputId, setTmdbInputId] = useState("");
  ...
  const handleSelectTmdbMovie = async (tmdbId) => {
    ...
    try {
      ...
      if (data && data.movieId) {
        ...
        setTmdbInputId(String(data.movieId));
        setSearchQuery(data.title);
      }
    } catch (err) {
      console.error("Failed to load TMDB details:", err);
      alert("Gagal mengambil data film dari TMDB.");
    } finally {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Gagal menyimpan movie");
      }
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-red-500 flex items-center gap-2">
            <i className="fa-solid fa-film" /> Tambah Film Baru
          </h1>
          <p className="text-zinc-400 text-sm mt-1">
            Cari film dari TMDB untuk auto-fill data lengkap, lalu masukkan link
            video film.
          </p>
        </div>
      </div>

      {/* TMDB Quick Search / Loader Banner */}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          <div className="w-full md:w-2/3 relative">
            <label className="text-xs text-zinc-400 font-semibold mb-1 flex items-center gap-1.5 uppercase tracking-wider">
              ...
              <span>Cari di TMDB (Auto-fill Metadata Film)</span>
            </label>
            <div className="relative">
              <input
                type="text"
                placeholder="Ketik judul film (contoh: Avatar, Spider-Man, Oppenheimer)..."
                value={searchQuery}
                onChange={(e) => handleSearch(e.target.value)}
                className="w-full px-4 py-2.5 bg-black/60 border border-zinc-600 rounded-xl text-sm focus:border-red-500 focus:outline-none placeholder-zinc-500"
              />
              {isSearching && (
                <div className="absolute right-3 top-3 text-xs text-zinc-400">
                  <i className="fa-solid fa-spinner fa-spin" /> Mencari...
                </div>
              )}
            </div>
            ...
            {searchResults.length > 0 && (
              <div className="absolute left-0 right-0 top-full mt-2 bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl max-h-80 overflow-y-auto z-50 divide-y divide-zinc-800">
                {searchResults.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => handleSelectTmdbMovie(item.id)}
                  ...
                  >
                    ...
                    <span className="text-xs px-2 py-1 rounded bg-red-600/80 text-white flex-shrink-0">
                      Pilih
                    </span>
                  </div>
                ))}
          ...
          {/* Direct TMDB ID Loader */}
          <div className="w-full md:w-1/3 flex gap-2 items-end">
            <div className="flex-1">
              <label className="text-xs text-zinc-400 font-semibold mb-1 block uppercase tracking-wider">
                Atau Masukkan TMDB ID
              </label>
              <input
                type="number"
                placeholder="TMDB ID (misal: 550)"
                value={tmdbInputId}
                onChange={(e) => setTmdbInputId(e.target.value)}
                className="w-full px-3 py-2.5 bg-black/60 border border-zinc-600 rounded-xl text-sm focus:border-red-500 focus:outline-none"
              />
            </div>
            <button
              type="button"
              onClick={() => tmdbInputId && handleSelectTmdbMovie(tmdbInputId)}
              disabled={!tmdbInputId || isLoadingDetails}
              className="px-4 py-2.5 bg-zinc-700 hover:bg-zinc-600 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition cursor-pointer"
            >
              {isLoadingDetails ? "Loading..." : "Terapkan"}
            </button>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={save} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white/5 p-6 rounded-2xl border border-white/10 space-y-5">
          <h2 className="text-lg font-bold border-b border-zinc-700 pb-3 flex items-center gap-2">
            <i className="fa-solid fa-pen-nib text-red-500" /> Informasi Film
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Auto-filled dari TMDB"
          ...
          >
            <i className="fa-solid fa-floppy-disk" /> Simpan Film ke Database
          </button>
        </div>
